...
- (void)viewDidLoad
{
    ...
    xAxisDown.labelPosition = XAxisLabelPositionBottom;
    xAxisDown.labelTextColor= UIColor.whiteColor;
    
    NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
    formatter.numberStyle = NSNumberFormatterScientificStyle;
    formatter.maximumFractionDigits = 3;
    [_chartView.xAxis setValueFormatter:[[ChartDefaultAxisValueFormatter alloc] initWithFormatter:formatter]];
    ...
    [self updateChartData:1000 set1YData:initData xCoefficient:1];
...
- (void)set1DataCount:(int)count yData:(float *)data xCoefficient: (float) coefficient
{
    ...
    {
        set1 = [[LineChartDataSet alloc] initWithValues:values  label:@"TimeDomain Spectrum"];
...
{
    return [_chartView lowestVisibleX];
}

- (float)readInViewStop
{
    return [_chartView highestVisibleX];
}

/** Extrat Data From The Zoom In View */
- (int)extractDataInView
{
    ...
    startIndex = (int)([_chartView lowestVisibleX]/xAxisCoefficient);
    stopIndex  = (int)([_chartView highestVisibleX]/xAxisCoefficient);
    viewDataSize = stopIndex - startIndex+1;
    
    memset(receivedWifiData.inViewData, 0, 1000*sizeof(float));
    
    for(int i =0; i<viewDataSize;i++)
    {
      receivedWifiData.inViewData[i] = receivedWifiData.spectrumData[startIndex+i];
    }
